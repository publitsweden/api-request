...
import ApiRequest, { isApiRequestError } from './ApiRequest';
import fetch from 'jest-fetch-mock';
describe('Request', () => {
    beforeEach(() => {
        ApiRequest.defaultOptions = {
    ...
    describe('defaultOptions', () => {
        it('should allow setting `origin` option', () => {
            ApiRequest.defaultOptions.origin = 'https://test.publit.com';
            expect(new ApiRequest('endpoint').url.origin).toBe('https://test.publit.com');
        });
        it('should allow setting `api` option', () => {
            ApiRequest.defaultOptions.api = 'test/v3.5';
            expect(new ApiRequest('endpoint').url.pathname).toBe('/test/v3.5/endpoint');
        });
        it('should allow setting `headers` option', () => {
            ...
            expect(new ApiRequest('endpoint').requestInit.headers).toEqual({
                'X-Test': 'test',
                'X-Test2': 'test2',
            });
        });
    ...
    describe('constructor', () => {
        it('should set `url` property', () => {
            expect(new ApiRequest('endpoint').url.toString()).toEqual('https://api.publit.com/publishing/v2.0/endpoint');
        });
        it('should set `requestInit` property', () => {
            expect(new ApiRequest('endpoint').requestInit).toEqual({
                method: 'GET',
                headers: {
                    'Content-Type': 'application/json',
                },
        ...
        it('should allow overriding `origin` option', () => {
            expect(new ApiRequest('endpoint').url.origin).toBe('https://api.publit.com');
            expect(new ApiRequest('endpoint', {
                origin: 'http://api.example.com:1992',
            }).url.origin).toBe('http://api.example.com:1992');
        });
        it('should allow overriding `api` option', () => {
            expect(new ApiRequest('endpoint').url.pathname).toBe('/publishing/v2.0/endpoint');
            expect(new ApiRequest('endpoint', {
                api: '',
    ...
    describe('fromUrl()', () => {
        it('should create a request object from a URL', () => {
            const request = ApiRequest.fromUrl('https://api.publit.com/endpoint');
    ...
    describe('limit()', () => {
        it('should set limit', () => {
            const r = new ApiRequest('endpoint').limit(10);
            expect(r.url.search).toBe('?limit=10');
        });
        it('should set limit and offset', () => {
            const r = new ApiRequest('endpoint').limit(10, 20);
            expect(r.url.search).toBe('?limit=20%2C10');
        });
        it('should overwrite existing limit', () => {
            const r = ApiRequest.fromUrl('https://example.com?limit=50');
            r.limit(10);
            expect(r.url.search).toBe('?limit=10');
        });
    });
    describe('with()', () => {
        it('should set single relation', () => {
            const r = new ApiRequest('endpoint').with('foo');
            expect(r.url.search).toBe('?with=foo');
        });
        it('should set multiple relations', () => {
            const r = new ApiRequest('endpoint').with('foo').with('bar').with('baz');
            expect(r.url.search).toBe('?with=foo%2Cbar%2Cbaz');
        });
        it('should add to existing relations', () => {
            const r = ApiRequest.fromUrl('https://example.com?with=foobar,bazoo');
    ...
    describe('has()', () => {
        it('should add an EQUAL relation filter', () => {
            const r = new ApiRequest('endpoint').has('work', 'title', 'EQUAL', 'Röda Rummet', 'AND');
            expect(r.url.search).toBe('?has=work%28title%3BEQUAL%3BR%C3%B6da+Rummet%29%3BAND');
        });
        it('should add a LIKE relation filter', () => {
            const r = new ApiRequest('endpoint').has('work', 'title', 'LIKE', 'Röda Rummet', 'AND');
        ...
        it('should add a relation filter for an array relation', () => {
            const r = new ApiRequest('endpoint').has('works', 'title', 'EQUAL', 'Röda Rummet');
            expect(r.url.search).toBe('?has=works%28title%3BEQUAL%3BR%C3%B6da+Rummet%29%3BOR');
        });
    });
    describe('groupBy()', () => {
        it('should set single groupBy', () => {
            const r = new ApiRequest('endpoint').groupBy('status');
    ...
    describe('where()', () => {
        it('should add an EQUAL attribute filter', () => {
            const r = new ApiRequest('endpoint').where('title', 'EQUAL', 'Röda Rummet', 'AND');
            expect(r.url.search).toBe('?title=R%C3%B6da+Rummet&title_args=EQUAL%3BAND');
        });
        it('should add a LIKE attribute filter', () => {
            const r = new ApiRequest('endpoint').where('title', 'LIKE', 'Röda Rummet', 'AND');
    ...
    describe('auxiliary()', () => {
        it('should set an auxiliary attribute', () => {
            const r = new ApiRequest('endpoint').auxiliary('extra');
    ...
    describe('orderBy()', () => {
        it('should set sort order', () => {
            const r = new ApiRequest('endpoint').orderBy('title');
            expect(r.url.search).toBe('?order_by=title');
        });
        it('should set sort order and direction', () => {
            const r = new ApiRequest('endpoint').orderBy('title', 'DESC');
    ...
    describe('index()', () => {
        it('should make a request', () => __awaiter(void 0, void 0, void 0, function* () {
            ...
            const request = yield new ApiRequest('things')
    ...
    describe('show()', () => {
        it('should make a show request', () => __awaiter(void 0, void 0, void 0, function* () {
            ...
            const request = yield new ApiRequest('things').show('123321');
    ...
    describe('store()', () => {
        it('should make a store request', () => __awaiter(void 0, void 0, void 0, function* () {
            fetch.mockResponse(JSON.stringify({ id: '123321' }));
            const request = yield new ApiRequest('things').store({
                hello: 'goodbye',
            });
            expect(request).toMatchObject({ id: '123321' });
    ...
    describe('update()', () => {
        it('should make an update request', () => __awaiter(void 0, void 0, void 0, function* () {
            fetch.mockResponse(JSON.stringify({ id: '123321' }));
            const request = yield new ApiRequest('things').update('123321', {
                hello: 'goodbye',
            });
            expect(request).toMatchObject({ id: '123321' });
            expect(fetch).toHaveBeenLastCalledWith('https://api.publit.com/publishing/v2.0/things/123321', expect.objectContaining({
    ...
    describe('delete()', () => {
        it('should make a delete request', () => __awaiter(void 0, void 0, void 0, function* () {
            fetch.mockResponse(JSON.stringify({ id: '123321' }));
            const request = yield new ApiRequest('things').delete('123321');
            expect(request).toMatchObject({ id: '123321' });
            expect(fetch).toHaveBeenLastCalledWith('https://api.publit.com/publishing/v2.0/things/123321', expect.objectContaining({
                method: 'DELETE',
            }));
    ...
    it('should allow changing RequestInit before fetching', () => __awaiter(void 0, void 0, void 0, function* () {
        fetch.mockResponse('{}');
        const request = new ApiRequest('things');
